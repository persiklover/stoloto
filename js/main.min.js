...
$(function () {
  ...
  $(".slider").addClass("enabled").addClass("owl-carousel").owlCarousel({
    items: 1,
    margin: 1,
    loop: true,
    mouseDrag: false,
    dots: true,
    nav: true,
    navText: ["<img src='img/arrow.png' aria-label='Вперед'>", "<img src='img/arrow.png' aria-label='Назад'>"],
    autoHeight: true,
    responsive: {
      0: {},
      768: {}
    ...
    success: function success(content) {
      ...
      function updateLottery(selector, data) {
        ...
        $(selector).each(function (i, el) {
          ...
          $(el).find(".js-super-prize").text(formatNumber(data.completedDraw.superPrize)).attr("aria-label", num2text(data.completedDraw.superPrize)).next().attr("aria-label", num2str(data.completedDraw.superPrize, ["рубль", "рубля", "рублей"])).attr("title", num2str(data.completedDraw.superPrize, ["рубль", "рубля", "рублей"]));
          ...
          start = msToTime(start).split(":").map(function (h) {
            return +Math.floor(Number(h));
          });
          $(el).find(".js-time").text("\n              ".concat(start[0], " ").concat(num2str(start[0], ["час", "часа", "часов"]), "\n              ").concat(start[1], " ").concat(num2str(start[0], ["минута", "минуты", "минут"]), "\n            "));
        });
      }
